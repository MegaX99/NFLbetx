...
create index pools_commissioner_id_idx on public.pools(commissioner_id);
...
create or replace function private.can_access_pool(check_pool_id uuid)
returns boolean
language sql
stable
security definer
...
create or replace function private.is_pool_commissioner(check_pool_id uuid)
returns boolean
language sql
stable
security definer
...
revoke execute on function private.can_access_pool(uuid) from public, anon, authenticated, service_role;
revoke execute on function private.is_pool_commissioner(uuid) from public, anon, authenticated, service_role;
...
to authenticated
using ((select private.can_access_pool(id)));
...
with check (
  ...
  and (select private.can_access_pool(pool_id))
);
...
using (
  ...
  or exists (
    ...
    where e.id = entry_id
      ...
      and (select private.can_access_pool(e.pool_id))
  )
);
